...
IPaintDevice* RLL::CreatePaintDeviceForFrame(IFrame* f)
{
	return new D3D12PaintDevice(f);
}
...
D3D12PaintDevice::D3D12PaintDevice(RLL::IFrame* f) : D3D12PaintDevice()
{
	Frame* wf = dynamic_cast<Frame*>(f);
	if (!wf)
	{
		SetError("Not a win32 impl of IFrame");
		return;
	}
	...
	SUCCESS(dxgiFactory->CreateSwapChainForComposition(cmdQueue.Get(), &scDesc, nullptr, &swapChain));
	SUCCESS(d3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&gFence)));
	ResizeView(SizeI(rc.right - rc.left, rc.bottom - rc.top));

	SUCCESS(dCompDevice->CreateTargetForHwnd(wf->hWnd, true, &dcompTarget));
...
void D3D12PaintDevice::ResizeView(SizeI& sz)
{
...
void D3D12PaintDevice::CreateRenderTargets()
{
...
{
	if (flags & FLAG_D12PC_FRAME_CONTEXT)
	{
		auto ctx = new D3D12FramePaintContext(this);
